...
@section('title', 'Request Perpanjang '.$transaksi->invoices_number  )
                    ...
                    <div class="normal-table-list">
                        <div class="bsc-tbl">
                            ...
                            <table class="table table-hover">
                                <tbody>
                                    <form action="{{ url('/transaksi/approved', $transaksi->invoices_number) }}" method="POST">
                                    @csrf
                                        <tr>
                                        ...
                                        <tr>
                                            <th><b>Tanggal Kembali Lama</b></th>
                                            <td> : </td>
                                            <td>{{Carbon\Carbon::parse($transaksi->tgl_kembali_lama)->format('d F Y')}}</td>
                                            <input id="tgl_kembali_lama" type="hidden" name="tgl_kembali_lama" 
                                        value="{{$transaksi->tgl_kembali_lama}}">
                                        </tr>
                                        <tr>
                                            <th><b>Tanggal Kembali Baru</b></th>
                                            <td> : </td>
                                            <td><strong>{{Carbon\Carbon::parse($transaksi->tgl_kembali)->format('d F Y')}}</strong></td>
                            ...
                            <table class="table table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <tr>
                                            <td colspan="4"><button type="submit" onclick="return confirm('Setujui permintaan perpanjang sewa?');" class="btn btn-primary"><i class="fas fa-check"></i>   Setujui</button></td>
                                        </form>

                                        <form action="{{route('transaksi.tolakperpanjang', $transaksi->invoices_number)}}" method="POST">
                                        @csrf
                                        <input id="tgl_kembali_lama" type="hidden" name="tgl_kembali_lama" 
                                        value="{{$transaksi->tgl_kembali_lama}}">
                                        
                                            <td><button type="submit" onclick="return confirm('Tolak permintaan perpanjang sewa?');" class="btn btn-danger"><i class="fas fa-trash"></i>     Tolak</button></td>
                                        </form>
                                        </tr>
                            </table>
                        </div>
